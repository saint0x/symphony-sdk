import { CommandMapProcessor } from './map-processor';
 ...
 */
export class ContextIntelligenceAPI {
    ...
    /**
     * Register Tool NLP Mapping - Internal method for auto-population
     */
    async registerToolNLPMapping(
        ...
        try {
            this.logger.info('ContextIntelligenceAPI', 'Registering tool NLP mapping', {
                toolName,
                nlpPattern: nlpPattern.substring(0, 50) + '...'
            });

            // Create pattern for command map
            const pattern = {
                id: `auto_${toolName}_${Date.now()}`,
                nlpPattern,
                toolName,
            ...
            // Add to command map processor
            await this.mapProcessor.addRuntimePattern(nlpPattern, {
                name: toolName,
                parameters: {},
                confidence: 0.7
            });

            this.logger.info('ContextIntelligenceAPI', 'Tool NLP mapping registered successfully', {
                toolName,
                patternId: pattern.id
            });

        } catch (error) {
            this.logger.error('ContextIntelligenceAPI', 'Failed to register tool NLP mapping', {
                error,
                toolName,
                nlpPattern
            });
